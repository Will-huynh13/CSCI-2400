...
cs1300bmp_readfile(char *filename, struct cs1300bmp *image)
{
  ...
  if ( error ) {
	...
	image -> color[row][col][COLOR_RED  ] = rarray[row * width + col]; // changed the order of the array, colors moved to the end
...
cs1300bmp_writefile(char *filename, struct cs1300bmp *image)
{
  ...
  for (int row = 0; row < height; row ++ ) {
    for (int col = 0; col < width; col ++ ) {
      rarray[row * width + col] = image -> color[row][col][COLOR_RED  ]; // changed the order of the array, moving colors to the
      garray[row * width + col] = image -> color[row][col][COLOR_GREEN]; // end
